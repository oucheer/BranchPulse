...
beforeEach(async () => {
  ...
  port = new ConfigPortService(source, () => '0.3')
})
...
function seedSource(): void {
  ...
  source.insert('monitoring_rules_repo', { repository_id: 'repo-1', enabled: 1, stale_threshold_days: 180, stale_threshold_unit: 'days', fetch_enabled: 1, naming_enabled: 1, email_policy: 'summary', notification_enabled: 1, auto_delete_enabled: 0, notify_target: 'both' })
...
describe('config import', () => {
  it('restores the exported state on a clean database', async () => {
    ...
    const freshPort = new ConfigPortService(fresh, () => '0.3')
  ...
  it('keeps local credentials that are not part of the bundle', async () => {
    ...
    const localPort = new ConfigPortService(local, () => '0.3')
  ...
  it('exports every repository, not only the active one', async () => {
    ...
    source.insert('monitoring_rules_repo', { repository_id: 'repo-2', enabled: 1, stale_threshold_days: 200, stale_threshold_unit: 'days', fetch_enabled: 1, naming_enabled: 1, email_policy: 'summary', notification_enabled: 1, auto_delete_enabled: 0, notify_target: 'self' })

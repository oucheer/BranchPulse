...
const getTransform = (rotateX: number, rotateY: number): string => `rotateX(${rotateX.toFixed(3)}deg) rotateY(${rotateY.toFixed(3)}deg)`
...
const DepthText = ({
  ...
  const rootStyle = {
    ...
    '--depth-text-shadow': shadow
      ? `0 1px 0 color-mix(in srgb, ${faceColor} 10%, transparent), 0 2px 12px color-mix(in srgb, ${depthColor} 24%, transparent), 0 12px 28px color-mix(in srgb, ${depthColor} 12%, transparent)`
      : 'none'
  } as CSSProperties

...
function Kill-All {
  Get-CimInstance Win32_Process | Where-Object { $_.Name -in @('GitManager.exe', 'gitmanager-stock.exe', 'electron.exe', 'GitManager-restored.exe') } | ForEach-Object {
...
function Test-Exe {
  ...
  $procs = Get-CimInstance Win32_Process | Where-Object { $_.Name -like '*GitManager*' -or $_.Name -eq 'electron.exe' }
  $renderers = $procs | Where-Object { $_.CommandLine -like '*--type=renderer*' }
  $any = Get-Process -Name GitManager, GitManager-restored, gitmanager-stock, electron -ErrorAction SilentlyContinue
...
Test-Exe -Exe (Join-Path $unpacked "GitManager-restored.exe") -Tag "restored" -ExtraArgs @("--no-sandbox")
Test-Exe -Exe (Join-Path $unpacked "gitmanager-stock.exe") -Tag "stock" -ExtraArgs @("--no-sandbox")
Kill-All
Write-Output "DONE"

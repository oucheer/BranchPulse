<#
Starts a packaged BranchPulse build against an isolated user-data directory so
...
Stop only the PID printed here; never blanket-kill BranchPulse processes.
#>
param(
  [string]$Exe = "$PSScriptRoot\..\release\win-unpacked\BranchPulse.exe",
  [string]$UserData = "$PSScriptRoot\..\.tmp-branchpulse-smoke\userdata",
  [string]$Port = "9222"
)
...
Write-Output "USERDATA=$env:BRANCHPULSE_USER_DATA_DIR"

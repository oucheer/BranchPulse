...
- 选分组有两种互不相同的语义，分别对应两个控件，**不要合并**：
  1. 「**分组范围**」（`GroupScopePicker`）管的是**检查 / 统计范围**：监控立即检查（`RunCheckOptions.groupIds`）、定时任务（`scheduler_jobs.group_ids`）、报告（`reports.group_ids`）与定时报告（`report_schedules.group_ids`）都可以只针对某些组做检查或统计，**空数组表示不限分组**。收窄逻辑只有 `shared/groups.ts` 的 `resolveGroupScope()` 一个出口。选中的组**全部**已被删除时必须收窄成空集合并记录失败（`report_group_scope_missing`），绝不能静默退回整仓——否则用户以为只查了某个组，实际整仓都被扫描并发信。
  2. 「**收件人里的组名**」管的是**通知范围**：`partitionRecipientTokens()`（`shared/groups.ts`）把收件人输入拆成「命中的组 + 其余普通收件人」，`MonitoringService.sendSummaryWithGroups()` 与 `ReportScheduleService.tick()` 都必须走它；若把组名直接丢给 `resolveRecipients()`，组名会被当成普通邮箱 token 混进汇总列表，语义就丢了。组名 token 的语义是「**把本次检查 / 报告的完整结果只发给这个组的组员**」，邮件内容**不按组收窄**——要让组员只收到本组数据，必须同时在「分组范围」里选中该组。每个组**单独发一封**（走 `GroupService.emailSummaryToGroup()`，`reportSchedule` 直接调 `groupRecipients()` + `sendReportEmail`），不要把多个组的组员并成一个收件人列表，否则各组会互相看到对方成员邮箱。
  两者可以叠加：只查这个组、也只通知这个组。`GroupService.emailBranches()`（组自己的分支情况发给组员）仍保留给设置页/分支列表的「发送给组员」按钮，不要拿它实现上面两种语义。
